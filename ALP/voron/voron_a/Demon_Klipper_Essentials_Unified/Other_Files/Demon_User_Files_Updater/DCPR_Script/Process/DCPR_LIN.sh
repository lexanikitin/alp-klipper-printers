...
# 3DPrintDemon Klipper Essentials Unified DCPR_LIN Script v1.0.0
...
var2=$(echo | grep -w $KOPF -e 'variable_linear_clean_start_x:')
var3=$(echo | grep -w $KOPF -e 'variable_linear_clean_start_y:')
var4=$(echo | grep -w $KOPF -e 'variable_linear_clean_start_z:')
var5=$(echo | grep -w $KOPF -e 'variable_linear_clean_axis:')
var6=$(echo | grep -w $KOPF -e 'variable_linear_clean_positive_dir:')
var7=$(echo | grep -w $KOPF -e 'variable_linear_clean_distance:')
...
var2target=$(echo | grep -w $DUF -e 'variable_linear_clean_start_x:')
var3target=$(echo | grep -w $DUF -e 'variable_linear_clean_start_y:')
var4target=$(echo | grep -w $DUF -e 'variable_linear_clean_start_z:')
var5target=$(echo | grep -w $DUF -e 'variable_linear_clean_axis:')
var6target=$(echo | grep -w $DUF -e 'variable_linear_clean_positive_dir:')
var7target=$(echo | grep -w $DUF -e 'variable_linear_clean_distance:')
...
trim_start5="${var5target%%[X-Y]*}"
trim_end5="${var5target#*[X-Y]}"
...
linetar2=$(echo | grep -nw $DUF -e 'variable_linear_clean_start_x:')
trim_line2="${linetar2%%:*}"

linetar3=$(echo | grep -nw $DUF -e 'variable_linear_clean_start_y:')
trim_line3="${linetar3%%:*}"

linetar4=$(echo | grep -nw $DUF -e 'variable_linear_clean_start_z:')
trim_line4="${linetar4%%:*}"

linetar5=$(echo | grep -nw $DUF -e 'variable_linear_clean_axis:')
trim_line5="${linetar5%%:*}"

linetar6=$(echo | grep -nw $DUF -e 'variable_linear_clean_positive_dir:')
trim_line6="${linetar6%%:*}"

linetar7=$(echo | grep -nw $DUF -e 'variable_linear_clean_distance:')
trim_line7="${linetar7%%:*}"
...
awk -v where="$trim_line9" -v what="$output9" 'FNR==where {print what; next} 1' $HDIR/tmp_file7.cfg > $HDIR/tmp_file8.cfg
wait
rm -f $HDIR/tmp_file7.cfg
awk -v where="$trim_line10" -v what="$output10" 'FNR==where {print what; next} 1' $HDIR/tmp_file8.cfg > $HDIR/tmp_file9.cfg
wait
rm -f $HDIR/tmp_file8.cfg
awk -v where="$trim_line11" -v what="$output11" 'FNR==where {print what; next} 1' $HDIR/tmp_file9.cfg > $HDIR/tmp_file10.cfg
wait
rm -f $HDIR/tmp_file9.cfg
awk -v where="$trim_line12" -v what="$output12" 'FNR==where {print what; next} 1' $HDIR/tmp_file10.cfg > $HDIR/tmp_file11.cfg
wait
